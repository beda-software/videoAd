...
void MainWindow::setBus(QHash<QString, int> buses) // TODO QHash in taskmanager
{
    this->bus_schedule->clear();

    QList<int> values = buses.values();
    ...
    for (int i = 0; i < count_item_in_bus_schedule; i++)
    {
        ...
        font.setBold(true);
        TableItemDelegate* bus = new TableItemDelegate(QString("%1").arg(values[i]), 0, font);
        this->bus_schedule->setItem(i+1, 0, bus);

        font.setPointSize(250/(int)scale);
        QTime t = QTime::fromString(buses[i], "hh:mm:ss"); // убрал секунды
        TableItemDelegate* next_time = new TableItemDelegate(QString(t.toString("hh:mm")),
                                                             1, font);
